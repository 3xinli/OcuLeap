        ...
        switch (slingshotState)
        {
            case SlingshotState.Idle:
				...
				if (swipeGesture == 1)
                {
					...
					Vector3 initialLocation = new Vector3(SlingshotMiddleVector.x - 2f, SlingshotMiddleVector.y, SlingshotMiddleVector.z );
					//Vector3 location = Camera.main.ScreenToWorldPoint(initialLocation );
					Vector3 location = initialLocation;
            ...
            case SlingshotState.UserPulling:
				...
				if (swipeGesture == 1 || swipeGesture == 2) // 1: down 2: up
                {
                    //get where user is tapping
                    //Vector3 location = Camera.main.ScreenToWorldPoint(Input.mousePosition);
					Vector3 currentLocation = BirdToThrow.transform.position;
					//float nx = storedX, ny = currentLocation.y - 0.5f * (swipeGesture - 1.5f) * 2.0f, nz = currentLocation.z;
					//float nx = currentLocation.x - 20f *(swipeGesture - 1.5f) * 2.0f, ny = currentLocation.y, nz = currentLocation.z;
					//Vector3 location = Camera.main.ScreenToWorldPoint(new Vector3(nx,ny,nz) );
					angle += ((float)swipeGesture - 1.5f)*0.05f;
					if (angle > 1.57)
						angle = 1.57;
					if (angle < -1.57)
						angle = -1.57;
					float nx = SlingshotMiddleVector.x - 1.5f * (float)Math.Cos(angle), ny = SlingshotMiddleVector.y - 1.5f * (float)Math.Sin(angle), nz = SlingshotMiddleVector.z;
					Vector3 location = new Vector3(nx,ny,nz);
                    location.z = 0;
                    //we will let the user pull the bird up to a maximum distance
					location = (location - SlingshotMiddleVector).normalized * 1.5f + SlingshotMiddleVector;
					BirdToThrow.transform.position = location;
					/*
				...
				else if (swipeGesture == 3)//user has removed the tap 
                {
